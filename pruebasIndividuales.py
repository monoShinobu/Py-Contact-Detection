...
qu0 = v.sphere(pos = v.vector(1,2,3), color = v.color.orange, radius = 0.15)
...
while True:
    ...
    for i in range(0, len(parti1.vertices)):#buscamos el vertice mas cercano al plano
        proA = f.proyeccion(parti1.vertices[i], dNormal, d)
    ...
    for i in range(0, len(parti2.vertices)):
        ...
        proB = f.proyeccion(parti2.vertices[i], dNormal, d)
        vDistB = f.distancia(proB, parti2.vertices[i])
        
        if vDistB < disMenorB:
            Q0 = parti2.vertices[i] #vertice de A mas cercano al plano
            numVertB = i #guardamos el nombre del vertice (ejemplo, vertice 1)
    ...
    if np.array_equal(valMovi, valMovi0) != True:
        ...
        ariCerA, ariCerB = f.aristaMenor(numVertA, numVertB, parti1, parti2)
